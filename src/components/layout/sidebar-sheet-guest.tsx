                    ...
                    <CollapsibleContent className="mt-1" asChild>
                        <div className="flex flex-col pl-4 space-y-2">
                            {item.children.map((child: any, index: number) => (
                                <div className={clsx("flex items-center gap-1 rounded-xl px-3 hover:bg-accent", child.active ? 'bg-accent' : '')} key={index}>
                                    <IconChevronsRight size={12} key={index} />
                                    <Link
                                        key={index}
                                        href={child.path}
                                        className={clsx("mx-[-0.65rem] flex min-w-full items-stretch gap-4 rounded-xl py-2 text-gray-900 dark:text-gray-100")}
                                    >
